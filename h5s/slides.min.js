var PERMANENT_URL_PREFIX="h5s/",SLIDE_CLASSES=["far-past","past","current","next","far-next"],PM_TOUCH_SENSITIVITY=15,SLIDESHOW_MODE=1,curSlide;
...
function getCurSlideFromHash(){var a=parseInt(location.hash.substr(1));curSlide=a?a-1:0}function updateHash(){location.replace("#"+(curSlide+1))}
...
function addEventListeners(){document.addEventListener("keydown",handleBodyKeyDown,!1)}function activateSlideshow(){for(var a,b=0;a=document.getElementsByTagName("link")[b];b++)if(-1!=a.getAttribute("rel").indexOf("style")){var c=a.getAttribute("title");if(c&&(a.disabled=!0,"Slideshow"==c))a.disabled=!1}SLIDESHOW_MODE=1;addMeta();setupFrames();updateSlides()}
function deactivateSlideshow(){for(var a=document.getElementsByTagName("link"),b,c=0;b=a[c];c++)if(-1!=b.getAttribute("rel").indexOf("style")){var d=b.getAttribute("title");if(d&&(b.disabled=!0,"All at once"==d))b.disabled=!1}curSlide=0;updateSlides();updateHash();SLIDESHOW_MODE=0;rmMeta();a=document.querySelectorAll("iframe");for(c=0;b=a[c];c++)enableFrame(b)}
function addMeta(){var a=document.createElement("meta");a.name="viewport";a.content="width=1100,height=750";document.querySelector("head").appendChild(a)}function rmMeta(){for(var a=document.getElementsByTagName("meta"),b,c=0;b=a[c];c++)"viewport"==b.name&&document.querySelector("head").removeChild(b)}function makeBuildLists(){for(var a=curSlide,b;b=slideEls[a];a++){b=b.querySelectorAll(".build > *");for(var c=0,d;d=b[c];c++)d.classList&&d.classList.add("to-build")}}
function handleDomLoaded(){slideEls=document.querySelectorAll("section.slides > article");addEventListeners();SLIDESHOW_MODE&&activateSlideshow();setupInteraction();makeBuildLists();document.body.classList.add("loaded")}function initialize(){getCurSlideFromHash();window._DCL?handleDomLoaded():document.addEventListener("DOMContentLoaded",handleDomLoaded,!1)}initialize();
