...
function getCurSlideFromHash(){var a=parseInt(location.hash.substr(1));!a&&SLIDESHOW_MODE&&(a=1);if(a){if(1>a)a=1;else if(a>slideEls.length)a=slideEls.length;curSlide=a-1;activateSlideshow()}else curSlide=0}function handleHashChange(){var a=parseInt(location.hash.substr(1));if(SLIDESHOW_MODE)if(a){if(1>a)a=1;else if(a>slideEls.length)a=slideEls.length;for(;curSlide<a-1;)nextSlide();for(;curSlide>a-1;)prevSlide()}else deactivateSlideshow();else getCurSlideFromHash()}
