...
1);enableSlideFrames(curSlide+2);isChromeVoxActive()&&speakAndSyncToNode(slideEls[curSlide]);updateHash()}function buildNextItem(){var a=slideEls[curSlide].querySelectorAll(".to-build");if(!a.length)return!1;var b=slideEls[curSlide].querySelectorAll(".just-built");a[0].classList.remove("to-build");a[0].classList.add("just-built");b.length&&b[0].classList.remove("just-built");isChromeVoxActive()&&speakAndSyncToNode(a[0]);return!0}
function prevSlide(){SLIDESHOW_MODE&&0<curSlide&&(curSlide--,updateSlides())}function nextSlide(){SLIDESHOW_MODE&&!buildNextItem()&&curSlide<slideEls.length-1&&(curSlide++,updateSlides())}function triggerEnterEvent(a){var b=getSlideEl(a);if(b){var c=b.getAttribute("onslideenter");c&&(new Function(c)).call(b);c=document.createEvent("Event");c.initEvent("slideenter",!0,!0);c.slideNumber=a+1;b.dispatchEvent(c)}}
function triggerLeaveEvent(a){var b=getSlideEl(a);if(b){var c=b.getAttribute("onslideleave");c&&(new Function(c)).call(b);c=document.createEvent("Event");c.initEvent("slideleave",!0,!0);c.slideNumber=a+1;b.dispatchEvent(c)}}function handleTouchStart(a){if(1==a.touches.length)touchDY=touchDX=0,touchStartX=a.touches[0].pageX,touchStartY=a.touches[0].pageY,document.body.addEventListener("touchmove",handleTouchMove,!0),document.body.addEventListener("touchend",handleTouchEnd,!0)}
function handleTouchMove(a){1<a.touches.length?cancelTouch():(touchDX=a.touches[0].pageX-touchStartX,touchDY=a.touches[0].pageY-touchStartY)}function handleTouchEnd(){var a=Math.abs(touchDX),b=Math.abs(touchDY);a>PM_TOUCH_SENSITIVITY&&b<2*a/3&&(0<touchDX?prevSlide():nextSlide());cancelTouch()}function cancelTouch(){document.body.removeEventListener("touchmove",handleTouchMove,!0);document.body.removeEventListener("touchend",handleTouchEnd,!0)}
function disableSlideFrames(a){if(a=getSlideEl(a))for(var a=a.getElementsByTagName("iframe"),b=0,c;c=a[b];b++)disableFrame(c)}function enableSlideFrames(a){if(a=getSlideEl(a))for(var a=a.getElementsByTagName("iframe"),b=0,c;c=a[b];b++)enableFrame(c)}function disableFrame(a){a.src="about:blank"}function enableFrame(a){var b=a._src;if(a.src!=b&&"about:blank"!=b)a.src=b}
function setupFrames(){for(var a=document.querySelectorAll("iframe"),b=0,c;c=a[b];b++)c._src=c.src,disableFrame(c);enableSlideFrames(curSlide);enableSlideFrames(curSlide+1);enableSlideFrames(curSlide+2)}
...
function getCurSlideFromHash(){var a=parseInt(location.hash.substr(1));SLIDESHOW_MODE&&!a&&(a=1);if(a){!a&&SLIDESHOW_MODE&&(a=1);if(1>a)a=1;else if(a>slideEls.length)a=slideEls.length;curSlide=a-1;activateSlideshow()}else curSlide=0}function handleHashChange(){var a=parseInt(location.hash.substr(1));if(SLIDESHOW_MODE)if(a){if(1>a)a=1;else if(a>slideEls.length)a=slideEls.length;for(;curSlide<a-1;)nextSlide();for(;curSlide>a-1;)prevSlide()}else deactivateSlideshow();else getCurSlideFromHash()}
